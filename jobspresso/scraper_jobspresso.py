...
async def scrape_jobspresso_jobs():
    jobs = []
    ...
    async with async_playwright() as p:
        ...
        while True:
            show_more_div = await page.query_selector('a.load_more_jobs')
            k = await show_more_div.get_attribute('style')
            print(f"Current style attribute: {k}")
            if k == 'display: none;':
        ...
        for job in job_elements:
            try:
                ...
                job_data = {
                    ...
                    "company_tagline": (await (await job.query_selector("span.job_listing-company-tagline")).inner_text()).strip() if await job.query_selector("span.job_listing-company-tagline") else "",
    ...
    # Save to JSON file
    with open("jobspresso_jobs.json", "w", encoding="utf-8") as f:
        json.dump(jobs, f, indent=2, ensure_ascii=False)
    
    return jobs

async def main():
    scraped_jobs = await scrape_jobspresso_jobs()
